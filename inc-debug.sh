...
_var_replace_vars() {
    local vars
        ...
        case $1 in
            ...
            haproxy)
                vars="S_RSYSLOG_PORT S_SERVICE[mail] S_EMAIL_ADMIN _SOMAXCONN _HPX_PATH_SSL _HPX_CERTBOT_PORT _HPX_STATS_PORT _HPX_STATS_2_PORT _APA_DOM_FQDN S_DOM_NAME _HPX_CT_NAME _HPX_ACCESS_USER _HPX_ACCESS_PWD _HPX_ACCESS_URI _HPX_DNS_DEFAULT _SERVER_DEFAULT" ;;
            logrotate)
                vars="S_PATH_LOG S_HOST_PATH_LOG S_VM_PATH_LOG S_PATH_LOG_INSTALL S_PATH_LOG_SERVER" ;;
            mail)
                vars="S_SERVICE[mail] S_PATH_CONF_SSL S_DOM_FQDN S_EMAIL_TECH S_EMAIL_ADMIN _MEL_PF_PATH_SQL _MEL_PATH_SSL _MEL_PATH_VMAIL _MEL_PATH_LOCAL _MEL_PATH_SIEVE _MEL_DB_HOST _MEL_DB_NAME _MEL_DB_USER _MEL_DB_PWD _MEL_SSL_SCHEME _MEL_PFA_HASH _MEL_DB_PFA_USER _MEL_DB_PFA_PWD _MEL_CIDR _IPS_CLUSTER _MEL_VMAIL_USER _MEL_VMAIL_UID" ;;
            rspamd)
                vars="S_RSPAMD_PORT[proxy] S_RSPAMD_PORT[normal] S_RSPAMD_PORT[controller] S_CACHE_PORT_RW S_CACHE_PORT_RO _MEL_CTS_RDS _IPS_CLUSTER _MEL_CIDR _MEL_PATH_RSPAMD _MEL_PATH_DKIM" ;;
            mariadb)
