...
_var_replace_vars() {
    local vars
        ...
        case $1 in
            ...
            haproxy)
                vars="S_RSYSLOG_PORT S_PATH_CONF_SSL _SOMAXCONN _HPX_LCRYPT_PORT _HPX_STATS_PORT _HPX_STATS_2_PORT _HPX_DOMAIN_FQDN _HPX_CT_NAME _HPX_ACCESS_USER _HPX_ACCESS_PWD _HPX_ACCESS_URI _HPX_DNS_DEFAULT _HPX_LCRYPT_EMAIL _SERVER_DEFAULT" ;;
            logrotate)
                vars="S_PATH_LOG S_HOST_PATH_LOG S_VM_PATH_LOG S_PATH_LOG_INSTALL S_PATH_LOG_SERVER" ;;
            mail)
                vars="S_SERVICE[mail] S_PATH_CONF _SSL _DOMAIN_FQDN _EMAIL_TECH _EMAIL_ADMIN _PATH_SSL _PATH_VMAIL _PATH_LMAIL _PATH_SIEVE _DB_HOST _DB_NAME _DB_USER _DB_PWD _SSL_SCHEME _DB_PFA_USER _DB_PFA_PWD _CIDR _IPS_CLUSTER _VMAIL_USER _VMAIL_UID" ;;
            rspamd)
